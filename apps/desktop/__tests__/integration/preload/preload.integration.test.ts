...
let capturedZmeelAPI: Record<string, unknown> = {};
let capturedZmeelShell: Record<string, unknown> = {};
...
describe('Preload Script Integration', () => {
  beforeEach(async () => {
    ...
    mockExposeInMainWorld.mockImplementation((name: string, api: unknown) => {
      if (name === 'zmeel') {
        capturedZmeelAPI = api as Record<string, unknown>;
      } else if (name === 'zmeelShell') {
        capturedZmeelShell = api as Record<string, unknown>;
      }
    });
  ...
  describe('API Exposure', () => {
    it('should expose zmeel API via contextBridge', () => {
      expect(mockExposeInMainWorld).toHaveBeenCalledWith('zmeel', expect.any(Object));
      expect(capturedZmeelAPI).toBeDefined();
    });

    it('should expose zmeelShell info via contextBridge', () => {
      expect(mockExposeInMainWorld).toHaveBeenCalledWith('zmeelShell', expect.any(Object));
      expect(capturedZmeelShell).toBeDefined();
    });

    it('should expose shell info with isElectron=true', () => {
      expect(capturedZmeelShell.isElectron).toBe(true);
    });

    it('should expose shell info with platform', () => {
      expect(capturedZmeelShell.platform).toBe(process.platform);
    });

    it('should expose shell info with version matching package.json', () => {
      expect(capturedZmeelShell.version).toBe(pkg.version);
    });
  });
  ...
  describe('IPC Method Invocations', () => {
    describe('App Info', () => {
      it('getVersion should invoke app:version', async () => {
        await (capturedZmeelAPI.getVersion as () => Promise<string>)();
        expect(mockInvoke).toHaveBeenCalledWith('app:version');
      });

      it('getPlatform should invoke app:platform', async () => {
        await (capturedZmeelAPI.getPlatform as () => Promise<string>)();
    ...
    describe('Shell Operations', () => {
      it('openExternal should invoke shell:open-external with URL', async () => {
        ...
        await (capturedZmeelAPI.openExternal as (url: string) => Promise<void>)(url);
    ...
    describe('Task Operations', () => {
      it('startTask should invoke task:start with config', async () => {
        ...
        await (
          capturedZmeelAPI.startTask as (config: { description: string }) => Promise<unknown>
        )(config);
        expect(mockInvoke).toHaveBeenCalledWith('task:start', config);
      });

      it('cancelTask should invoke task:cancel with taskId', async () => {
        await (capturedZmeelAPI.cancelTask as (taskId: string) => Promise<void>)('task_123');
      ...
      it('interruptTask should invoke task:interrupt with taskId', async () => {
        await (capturedZmeelAPI.interruptTask as (taskId: string) => Promise<void>)(
          'task_123',
        );
      ...
      it('removeFavorite should invoke favorites:remove with taskId', async () => {
        await (capturedZmeelAPI.removeFavorite as (taskId: string) => Promise<void>)(
          'task_123',
        );
      ...
      it('listFavorites should invoke favorites:list', async () => {
        await (capturedZmeelAPI.listFavorites as () => Promise<unknown[]>)();
        expect(mockInvoke).toHaveBeenCalledWith('favorites:list');
      });

      it('isFavorite should invoke favorites:has with taskId', async () => {
        await (capturedZmeelAPI.isFavorite as (taskId: string) => Promise<boolean>)(
          'task_123',
        );
    ...
    describe('Permission Operations', () => {
      it('respondToPermission should invoke permission:respond', async () => {
        ...
        await (
          capturedZmeelAPI.respondToPermission as (r: {
            taskId: string;
            allowed: boolean;
          }) => Promise<void>
        )(response);
    ...
    describe('Session Operations', () => {
      it('resumeSession should invoke session:resume', async () => {
        await (
          capturedZmeelAPI.resumeSession as (
            s: string,
            p: string,
            t?: string,
            attachments?: unknown[],
    ...
    describe('Settings Operations', () => {
      it('getDebugMode should invoke settings:debug-mode', async () => {
        await (capturedZmeelAPI.getDebugMode as () => Promise<boolean>)();
        expect(mockInvoke).toHaveBeenCalledWith('settings:debug-mode');
      });

      it('setDebugMode should invoke settings:set-debug-mode', async () => {
        await (capturedZmeelAPI.setDebugMode as (enabled: boolean) => Promise<void>)(true);
      ...
      it('getAppSettings should invoke settings:app-settings', async () => {
        await (capturedZmeelAPI.getAppSettings as () => Promise<unknown>)();
      ...
      it('getSlackMcpOauthStatus should invoke opencode:auth:slack:status', async () => {
        await (capturedZmeelAPI.getSlackMcpOauthStatus as () => Promise<unknown>)();
      ...
      it('loginSlackMcp should invoke opencode:auth:slack:login', async () => {
        await (capturedZmeelAPI.loginSlackMcp as () => Promise<unknown>)();
      ...
      it('logoutSlackMcp should invoke opencode:auth:slack:logout', async () => {
        await (capturedZmeelAPI.logoutSlackMcp as () => Promise<void>)();
    ...
    describe('Onboarding Operations', () => {
      it('getOnboardingComplete should invoke onboarding:complete', async () => {
        await (capturedZmeelAPI.getOnboardingComplete as () => Promise<boolean>)();
      ...
      it('setOnboardingComplete should invoke onboarding:set-complete', async () => {
        await (capturedZmeelAPI.setOnboardingComplete as (c: boolean) => Promise<void>)(true);
    ...
    describe('Model Operations', () => {
      it('getSelectedModel should invoke model:get', async () => {
        await (capturedZmeelAPI.getSelectedModel as () => Promise<unknown>)();
      ...
      it('setSelectedModel should invoke model:set', async () => {
        ...
        await (
          capturedZmeelAPI.setSelectedModel as (m: {
            provider: string;
            model: string;
          }) => Promise<void>
        )(model);
    ...
    describe('Logging Operations', () => {
      it('logEvent should invoke log:event', async () => {
        ...
        await (capturedZmeelAPI.logEvent as (p: unknown) => Promise<unknown>)(payload);
    ...
    describe('Skills Operations', () => {
      it('getUserSkillsPath should invoke skills:get-user-skills-path', async () => {
        await (capturedZmeelAPI.getUserSkillsPath as () => Promise<string>)();
  ...
  describe('Event Subscriptions', () => {
    it('onTaskUpdate should subscribe to task:update', () => {
      ...
      (capturedZmeelAPI.onTaskUpdate as (cb: (e: unknown) => void) => () => void)(callback);
    ...
    it('onTaskUpdate should return unsubscribe function', () => {
      const callback = vi.fn();
      const unsubscribe = (
        capturedZmeelAPI.onTaskUpdate as (cb: (e: unknown) => void) => () => void
      )(callback);
      unsubscribe();
    ...
    it('onTaskUpdateBatch should subscribe to task:update:batch', () => {
      const callback = vi.fn();
      (capturedZmeelAPI.onTaskUpdateBatch as (cb: (e: unknown) => void) => () => void)(
        callback,
      );
    ...
    it('onPermissionRequest should subscribe to permission:request', () => {
      const callback = vi.fn();
      (capturedZmeelAPI.onPermissionRequest as (cb: (e: unknown) => void) => () => void)(
        callback,
      );
    ...
    it('onTaskProgress should subscribe to task:progress', () => {
      const callback = vi.fn();
      (capturedZmeelAPI.onTaskProgress as (cb: (e: unknown) => void) => () => void)(callback);
      expect(mockOn).toHaveBeenCalledWith('task:progress', expect.any(Function));
    });

    it('onDebugLog should subscribe to debug:log', () => {
      const callback = vi.fn();
      (capturedZmeelAPI.onDebugLog as (cb: (e: unknown) => void) => () => void)(callback);
    ...
    it('onTaskStatusChange should subscribe to task:status-change', () => {
      const callback = vi.fn();
      (capturedZmeelAPI.onTaskStatusChange as (cb: (e: unknown) => void) => () => void)(
        callback,
      );
