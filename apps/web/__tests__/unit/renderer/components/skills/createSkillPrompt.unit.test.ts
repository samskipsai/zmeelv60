  ...
  describe('buildCreateSkillPrompt', () => {
    it('pins only the deterministic base path for unix paths', () => {
      const prompt = buildCreateSkillPrompt({
        ...
        skillsBasePath: '/Users/test/Library/Application Support/Zmeel/skills/',
        platform: 'darwin',
      });

      expect(prompt).toContain('/skill-creator');
      expect(prompt).toContain(
        'Use this exact base directory: `/Users/test/Library/Application Support/Zmeel/skills`',
      );
      expect(prompt).toContain(
    ...
    it('normalizes windows base path when platform is win32', () => {
      const prompt = buildCreateSkillPrompt({
        ...
        skillsBasePath: 'C:/Users/Test/AppData/Roaming/Zmeel/skills/',
        platform: 'win32',
      });

      expect(prompt).toContain(
        'Use this exact base directory: `C:\\Users\\Test\\AppData\\Roaming\\Zmeel\\skills`',
      );
    ...
    it('keeps unix paths with spaces in user home segments', () => {
      const prompt = buildCreateSkillPrompt({
        ...
        skillsBasePath: '/home/Jane Doe/.local/share/Zmeel/skills/',
        platform: 'linux',
      });

      expect(prompt).toContain(
        'Use this exact base directory: `/home/Jane Doe/.local/share/Zmeel/skills`',
      );
    });
    ...
    it('keeps windows paths with spaces in user names', () => {
      const prompt = buildCreateSkillPrompt({
        name: 'Windows Skill',
        description: 'Works on Windows',
        skillsBasePath: 'C:/Users/Jane Doe/AppData/Roaming/Zmeel/skills/',
        platform: 'win32',
      });

      expect(prompt).toContain(
        'Use this exact base directory: `C:\\Users\\Jane Doe\\AppData\\Roaming\\Zmeel\\skills`',
      );
    });
    ...
    it('preserves tilde-prefixed paths without expansion', () => {
      const prompt = buildCreateSkillPrompt({
        ...
        skillsBasePath: '~/.config/Zmeel/skills/',
        platform: 'linux',
      });

      expect(prompt).toContain('Use this exact base directory: `~/.config/Zmeel/skills`');
    });
    ...
    it('normalizes windows UNC network paths', () => {
      const prompt = buildCreateSkillPrompt({
        ...
        skillsBasePath: '//Server01/Zmeel/skills/',
        platform: 'win32',
      });

      expect(prompt).toContain('Use this exact base directory: `\\\\Server01\\Zmeel\\skills`');
    });
