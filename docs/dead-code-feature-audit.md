...
main `zmeel.db` (migration v030), and the retired `workspace-meta.db`
...
- Daemon `task-config-builder.ts` routes through `resolveTaskConfig` with
  `zmeelRuntime`, `zmeelStorageDeps`, `configFileName`, and the
...
- `apps/daemon/src/storage-service.ts` passes `legacyMetaDbPath` into
  ...
  workspace tables live in `zmeel.db` now.
...
| ------------------------------------------------------------ | ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| [#945](https://github.com/samskipsai/zmeelv60/pull/945) | Remove dead thought-stream reporting pipeline                           | Thought Stream runtime (docs closed by #950)                                                                                                |
| [#947](https://github.com/samskipsai/zmeelv60/pull/947) | Route daemon task config through resolveTaskConfig (restore 6 features) | Daemon resolver bypass (P1); GWS wiring gap (P1); connectors / cloud browser / knowledge notes / language / `store:false` (follow-up audit) |
| [#948](https://github.com/samskipsai/zmeelv60/pull/948) | Drop orphan files & unused ws dep (dead-code sweep P3)                  | `daemon/types.ts`, `daemon/cron-utils.ts`, `__scratch__/`, 4 desktop shims, `ws` daemon dep, eslint ignore cleanup                          |
| [#950](https://github.com/samskipsai/zmeelv60/pull/950) | Close thought-stream stale-docs item                                    | Thought-stream stale docs (P3) — `daemon-final-architecture.md` Owner table edit                                                            |
| [#951](https://github.com/samskipsai/zmeelv60/pull/951) | Drop orphan `apps/desktop/src/main/opencode/config-generator.ts`        | Desktop OpenCode config generator (P2) — −245 LOC, zero live callers across all 7 exports                                                   |
